...
alias SipHash24 = SipHash(2, 4)
alias SipHash13 = SipHash(1, 3)
...
# whereas `SipHash(1, 3)` is faster but not verified, but should only be used
...
struct SipHash(CROUNDS, DROUNDS)
  ...
  def self.siphash(input : Int | UInt | Float, key : Key) : UInt64
